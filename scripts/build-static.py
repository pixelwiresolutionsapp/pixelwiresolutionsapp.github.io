  ...
  function render() {
    ...
    if (loading) {
      ...
      filtered.forEach(p => {
        ...
        if (img) h += '<img src="'+img.url+'" alt="'+esc(p.name)+'" class="w-full h-full object-contain p-4 transition group-hover:scale-105" onerror="this.style.display=\'none\'" referrerpolicy="no-referrer" />';
    ...
    if (lightboxProduct) {
      ...
      if (lp.images[lightboxIdx]) h += '<img src="'+lp.images[lightboxIdx].url+'" alt="'+esc(lp.name)+'" class="max-h-80 object-contain" onerror="this.style.display=\'none\'" referrerpolicy="no-referrer" />';
